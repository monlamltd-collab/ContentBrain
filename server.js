...
app.post('/api/social/posts/:id/rerender', requireAuth, async (req, res) => {
  try {
    ...
    const { filename: imageFilename } = await renderPost(post.template_type, post.brand, post);
    let videoFilename = null;
    try {
      const video = await renderVideo(post.template_type, post.brand, post);
      videoFilename = video.filename;
    ...
    const updates = { image_url: imageFilename };
    if (videoFilename) updates.video_url = videoFilename;
