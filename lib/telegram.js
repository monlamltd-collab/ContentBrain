...
async function sendBlogForReview(item) {
  ...
  const buttons = {
    inline_keyboard: [
      [
        { text: 'Approve', callback_data: `rv:${item.content_type}:approve:${item.post_id}` },
        { text: 'Revise', callback_data: `rv:${item.content_type}:revise:${item.post_id}` },
        { text: 'Reject', callback_data: `rv:${item.content_type}:reject:${item.post_id}` }
      ]
    ]
  };
