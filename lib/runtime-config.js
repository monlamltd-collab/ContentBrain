...
// ── Template durations (seconds) ──────────────────────────────────────
// Per-template default video length, operator-tunable via the Design tab
// or the /durations Telegram command ('global','template_durations').
// Defaults mirror the composition declarations in video/Root.jsx so an
// unset lever changes nothing. Values clamp to the video-renderer's
// proven 3–90s render envelope.

const DEFAULT_TEMPLATE_DURATIONS = Object.freeze({ stat: 6, hook: 7, list: 8, reel: 6 });
const DURATION_MIN = 3;
const DURATION_MAX = 90;

async function getTemplateDurations() {
  const v = await readRaw('global', 'template_durations');
  const out = { ...DEFAULT_TEMPLATE_DURATIONS };
  if (v && typeof v === 'object' && !Array.isArray(v)) {
    for (const t of Object.keys(DEFAULT_TEMPLATE_DURATIONS)) {
      const n = Number(v[t]);
      if (Number.isFinite(n) && n > 0) {
        out[t] = Math.min(DURATION_MAX, Math.max(DURATION_MIN, Math.round(n)));
      }
    }
  }
  return out;
...
// ── Paid boost (Meta follower ads) levers ─────────────────────────────
// Real-money controls for the social-engine boost path. ALL default to the
// safe state: disabled, £2/day per boost, £50 cumulative ceiling. Mirrors
// the numeric defaults in lib/social-engine/constants.js (kept inline so this
// generic layer doesn't depend on the social-engine module). Operator tunes
// via the /boost Telegram command.

async function getBoostConfig() {
  const enabledRaw = await readRaw('global', 'boost.enabled');
  const budgetRaw = Number(await readRaw('global', 'boost.daily_budget_pence'));
  const capRaw = Number(await readRaw('global', 'boost.spend_cap_pence'));

  const dailyBudgetPence = Number.isFinite(budgetRaw) && budgetRaw > 0
    ? Math.min(2000, Math.max(50, Math.round(budgetRaw)))   // £0.50–£20/day
    : 200;
  const spendCapPence = Number.isFinite(capRaw) && capRaw > 0
    ? Math.min(100000, Math.max(100, Math.round(capRaw)))   // £1–£1000 ceiling
    : 5000;
  return {
    enabled: enabledRaw === true,   // strict — only a literal true enables spend
    dailyBudgetPence,
    spendCapPence,
  };
}
...
function reviewDeliveryKey(postId, brand = 'auctionbrain') {
  if (!/^[A-Za-z0-9_-]+$/.test(String(postId))) throw new Error('Invalid review post id');
  return `review.delivered.${brand}.${postId}`;
}

async function getReviewDeliveredAt(postId, brand = 'auctionbrain') {
  const value = await readRaw('global', reviewDeliveryKey(postId, brand));
  return typeof value === 'string' && value ? value : null;
}

async function markReviewDelivered(postId, brand = 'auctionbrain', deliveredAt = new Date().toISOString()) {
  await setLever('global', reviewDeliveryKey(postId, brand), deliveredAt);
  return deliveredAt;
}
