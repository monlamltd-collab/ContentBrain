...
const { pickLotOfTheDay, pickWeeklySuperlatives, WEEKLY_SUPERLATIVE_ORDER } = require('./lot-picker');
...
const { formatWeeklySummary } = require('./weekly-summary');
...
const WEEKLY_REEL_DURATION_SECONDS = 30;

const SUPERLATIVE_EMOJI = {
  'cheapest-week': '\u{1F4B7}',
  'dearest-week': '\u{1F3F0}',
  'best-deal-week': '\u{2B50}',
  'biggest-discount-week': '\u{1F4C9}',
  'worst-lot-week': '\u{1F648}',
};

/**
 * Weekly superlative reels — the "X of the week" series.
 *
 * One-stage flow (no voice memo, unlike runLotOfTheDay): pick the 5 superlative
 * lots, generate music-only content, render each as a 30s vertical reel, insert
 * a draft post with the video already attached + meta.is_reel=true, stagger
 * scheduled_for one weekday apart, and send each to Telegram for the standard
 * approve/revise/reject review. The 15-minute publish cron then publishes each
 * reel (via the /video_reels endpoint, because meta.is_reel is set) when its
 * scheduled_for time arrives.
 *
 * Pass archetypes to retry only a failed subset without recreating successful
 * posts. Returns { generated, failed, skipped, failures }.
 */
async function runWeeklySuperlatives({ archetypes } = {}) {
  console.log('[lot-flow] runWeeklySuperlatives starting...');

  const requested = Array.isArray(archetypes) ? [...new Set(archetypes)] : WEEKLY_SUPERLATIVE_ORDER;
  const picks = await pickWeeklySuperlatives({ superlatives: requested });
  if (!picks.length) {
    await sendNotification('Weekly reels: no qualifying lots found this week — nothing generated.');
    return { generated: 0, failed: 0, skipped: requested.length, failures: [] };
  }
  ...
  for (let i = 0; i < picks.length; i++) {
    const { superlative, lot } = picks[i];
    const frame = ARCHETYPE_FRAMES[superlative] || {};
    try {
      // eslint-disable-next-line no-await-in-loop
      const content = await generateLotContent({ lot, archetype: superlative });
      const bullets = (content.key_bullets || []).slice(0, 4);

      // Stagger publish one weekday apart at 10:00, starting today.
      const scheduledFor = new Date();
      const weekdayOffset = Math.max(0, WEEKLY_SUPERLATIVE_ORDER.indexOf(superlative));
      scheduledFor.setDate(scheduledFor.getDate() + weekdayOffset);
      scheduledFor.setHours(10, 0, 0, 0);

      // Render as image post — no more video reels. Each weekly pick gets a
            // hero-album style static post with the lot image + text overlay.
            const renderPayload = {
              copy_headline: content.hook_headline,
              copy_body: bullets.join('\n'),
              copy_cta: 'auctionbrain.co.uk',
              platform: 'facebook',
              meta: {
                niche_tag_label: frame.label || null,
                micro_stat: null,
                micro_caption: null,
              },
              visual_hints: {
            ...
            // eslint-disable-next-line no-await-in-loop
            const rendered = await renderPost('social-engine/hero-album', 'auctionbrain', renderPayload);
            console.log(`[lot-flow] rendered ${superlative}: ${rendered.filename}`);
            ...
            const post = await insertPost({
              ...
              meta: {
                ...
                superlative,
                superlative_badge: frame.label || null,
                weekly_series: true,
                is_reel: false,
                hook_headline: content.hook_headline,
                key_bullets: bullets,
                ...
                visual_style: content.visual_style,
                social_mode: 'monet',
                social_type: 'hero-album',
              },
            });

      // eslint-disable-next-line no-await-in-loop
      const sendResult = await sendPostForReview(post);
      if (!sendResult.ok) {
        console.error(`[lot-flow] sendPostForReview failed for ${superlative}: ${sendResult.error}`);
      }
      results.push({ superlative, postId: post.id });
    } catch (err) {
      console.error(`[lot-flow] weekly reel '${superlative}' failed: ${err.message}`);
      failures.push({ superlative, error: err.message });
    }
  }

  await sendNotification(formatWeeklySummary(results, failures));

  console.log(`[lot-flow] runWeeklySuperlatives done: ${results.length} generated, ${failures.length} failed`);
  return { generated: results.length, failed: failures.length, skipped: requested.length - picks.length, failures };
}

module.exports = { runLotOfTheDay, findPendingLotPost, processVoiceForLot, runWeeklySuperlatives };
