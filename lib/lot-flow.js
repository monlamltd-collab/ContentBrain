 ...
 */
async function runWeeklySuperlatives() {
  ...
  if (!picks.length) {
    ...
    return { generated: 0, failed: 0, skipped: 5 };
  }
  ...
  const lines = [
      `\\u{1F4F8} <b>Weekly auction posts</b> — ${results.length} ready to review`,
      ...results.map(r => `${SUPERLATIVE_EMOJI[r.superlative] || '\\u{2022}'} ${escHtml(ARCHETYPE_FRAMES[r.superlative]?.label || r.superlative)}`),
  ];
  if (failures.length) {
    lines.push('', `\u{26A0} ${failures.length} failed: ${escHtml(failures.map(f => f.superlative).join(', '))}`);
  }
  ...
  return { generated: results.length, failed: failures.length, skipped: 5 - picks.length };
}
