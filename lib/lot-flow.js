...
async function runWeeklySuperlatives() {
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    try {
      ...
      // Render now — music only, no voiceover. 30s keeps the existing LotVideo
      // bullet/CTA timing intact while staying a punchy reel length.
      const renderPost = {
        lot,
        archetype: superlative,
        hookHeadline: content.hook_headline,
        keyBullets: bullets,
        superlativeBadge: frame.label || null,
        theme: content.visual_style,
        overrideDurationSeconds: WEEKLY_REEL_DURATION_SECONDS,
        platform: 'facebook',
      };
      // eslint-disable-next-line no-await-in-loop
      const rendered = await renderVideo('lot', 'auctionbrain', renderPost);
  ...
  const lines = [
    `\u{1F3AC} <b>Weekly auction reels</b> — ${results.length} ready to review`,
