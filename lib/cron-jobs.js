...
//   05:45        Reddit scraper (OAuth API first) → scraped_articles + briefs
...
//   07:30        Phase G daily social-engine post
...
async function runGenerate({ force = false } = {}) {
  ...
  try {
    ...
    lastGenerateDate = null; // allow wake recovery/manual trigger to retry
...
let pendingWeeklyReelsRetry = null;
...
async function runWeeklyReels({ force = false } = {}) {
  ...
  try {
    ...
    const retryArchetypes = pendingWeeklyReelsRetry?.week === wk
      ? pendingWeeklyReelsRetry.archetypes
      : undefined;
    const result = await runWeeklySuperlatives(retryArchetypes ? { archetypes: retryArchetypes } : {});
    if (result && result.failed > 0) {
      const failedArchetypes = (result.failures || []).map(failure => failure.superlative).filter(Boolean);
      if (!failedArchetypes.length) {
        throw new Error(`${result.failed} weekly posts failed without retry details`);
      }
      pendingWeeklyReelsRetry = { week: wk, archetypes: [...new Set(failedArchetypes)] };
      lastWeeklyReelsWeek = null;
      console.warn(`[${new Date().toISOString()}] Weekly reels: ${failedArchetypes.length} failed; only those archetypes will retry.`);
      return result;
    }
    pendingWeeklyReelsRetry = null;
    return result;
  } catch (err) {
...
// Social generation is opt-in. The owner's editorial policy is blog-only:
// one AuctionBrain blog and one BridgeMatch blog on Monday/Wednesday for
// Wednesday/Friday publication. One switch guards every social producer so
// the old daily batches cannot rebuild a large approval queue.
const socialGenerationEnabled = process.env.ENABLE_SOCIAL_GENERATION === 'true';
if (socialGenerationEnabled) {
  cron.schedule('0 7 * * *', runGenerate);
}

// Deliver any blog/guide drafts that were queued off-cadence. The inbound
// review API sends same-day Mon/Wed engine output immediately; this sweep is
// for manual/off-day submissions and missed notifications.
cron.schedule('15 7 * * 1,3', async () => {
  console.log(`[${new Date().toISOString()}] Mon/Wed queued blog review sweep...`);
  await require('./telegram-handlers').resendDraftReviewCards();
...
// 07:00 generation window. Reddit OAuth API first (free, not IP-blocked) when
// REDDIT_CLIENT_ID/SECRET are set, else Crawlee-first with Firecrawl-enhanced
// bypass for 403-blocked pages. Pulls top weekly threads from the
...
cron.schedule('45 5 * * *', async () => {
  try {
    ...
    if (result.errors && result.errors.length >= 3) {
      const examples = result.errors.slice(0, 3)
        .map(error => `• ${String(error).replace(/[<>]/g, '').slice(0, 180)}`)
        .join('\n');
      const setupHint = result.source === 'crawlee'
        ? '\n\nOAuth is not configured; set REDDIT_CLIENT_ID and REDDIT_CLIENT_SECRET to avoid datacenter HTML blocks.'
        : '';
      await sendNotification(
        `<b>Reddit scraper</b>: ${result.inserted} inserted, ${result.errors.length} errors\n${examples}${setupHint}`
      ).catch(() => {});
    }
  } catch (err) {
...
// Check on wake — only when social generation has been explicitly enabled.
if (socialGenerationEnabled) cron.schedule('*/30 * * * *', async () => {
  const today = new Date().toISOString().slice(0, 10);
...
// Phase G — Daily social-engine post at 07:30 UTC.
//
...
//
// Phase G social-engine — daily post cron at 07:30 UTC. Staggered from the
// legacy 07:00 renderer to avoid simultaneous Chromium launches in Railway.
...
if (socialGenerationEnabled) cron.schedule('30 7 * * *', async () => {
  try {
...
if (socialGenerationEnabled) cron.schedule('0 8 * * 1', runWeeklyReels);

// Wake recovery — only when social generation is explicitly enabled.
if (socialGenerationEnabled) cron.schedule('*/30 * * * *', async () => {
  const now = new Date();
