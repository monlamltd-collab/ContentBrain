...
async function getDraftBlogPosts() {
  const { data, error } = await supabase
    ...
    .eq('status', 'draft')
    .order('created_at', { ascending: false });
  if (error) throw new Error(`Get draft blog posts failed: ${error.message}`);
  return data || [];
}

async function updateBlogPostStatus(id, status, extras = {}) {
...
module.exports = { supabase, insertPost, getDraftPosts, getApprovedPosts, updatePostStatus, getPostById, uploadMedia, saveBrief, getPendingBriefs, markBriefsUsed, getRecentApprovedPosts, getRecentRejectedPosts, getRecentPublishedPosts, saveSeed, getUnusedSeeds, markSeedsUsedForSocial, getDraftBlogPosts, updateBlogPostStatus, getBlogPostById };
