...
async function findLotsByArchetype(archetype, opts = {}) {
  ...
  const cols = 'id, house, lot_number, url, address, postcode, price, price_text, prop_type, beds, tenure, sqft, condition, image_url, images, bullets, auction_date, status, score, score_breakdown, opps, risks, deal_type, vacant, est_monthly_rent, est_annual_rent, est_gross_yield, street_avg, below_market, epc_rating, flood_risk';
  let query = supabase
    .from('lots')
    .select(cols)
