...
// Prefers the service key: ContentBrain is a trusted server-side worker, and the
// outreach tables (prospects, contacts, replies, sequences, suppression,
// outbound_outcomes, boost_runs, social_audience_daily) are getting RLS enabled
// with no anon policies — anon-key access to them will return empty/fail once
// that lands. Falls back to the anon key so local/dev setups keep working.
const supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_ANON_KEY
);
if (!process.env.SUPABASE_SERVICE_KEY) {
  console.warn('[supabase] SUPABASE_SERVICE_KEY not set — using anon key; outreach tables will be unreadable once RLS is enabled');
}
...
// One-time backlog cleanup used when automatic social generation is disabled.
// Outbound email drafts have platform=NULL, so the platform filter keeps them
// untouched while archiving Facebook/other social drafts. Rows are retained as
// rejected (not deleted), making the operation auditable and reversible.
async function archiveDraftSocialPosts(client = supabase, legacyCutoff = '2026-07-22T23:59:59.999Z') {
  // Fixed cut-off makes this a bounded cleanup of the backlog that existed when
  // the blog-only policy was adopted. Restarts can never reject later manual
  // drafts, even if they have a social platform.
  const { data, error } = await client
    ...
    .lte('created_at', legacyCutoff)
    .select('id');
  if (error) throw new Error(`Archive social drafts failed: ${error.message}`);
  return (data || []).length;
}
 ...
 *
 * Window = traditional live inventory only (real auction_date >= today and
 * < 2090 — excludes MMOA sentinel 2099-12-31). Lot-of-the-day content is
 * date/urgency oriented; MMOA continuous stock is pulled separately for
 * generic social material via getFreshLotsForMaterial.
 *
 * Score floor defaults keep daily rotation viable on real inventories.
 */
async function findLotsByArchetype(archetype, opts = {}) {
  const limit = opts.limit ?? 20;
  const minScore = opts.minScore ?? 6;
  const today = todayUTC();
  const horizon14 = daysAheadUTC(14);
  const horizon7 = daysAheadUTC(7);
  ...
  switch (archetype) {
    case 'best-yield':
      result = await queryLiveLots(supabase, {
        ...base,
        horizonEnd: horizon14,
        minScore,
        order: { column: 'est_gross_yield', ascending: false },
        refine: (q) => q.gte('est_gross_yield', 8),
      });
      break;
    case 'deepest-discount':
      result = await queryLiveLots(supabase, {
        ...
        horizonEnd: horizon14,
        minScore,
        order: { column: 'below_market', ascending: false },
        refine: (q) => q.not('below_market', 'is', null).gt('below_market', 0),
      });
      break;
    case 'dev-or-refurb':
      result = await queryLiveLots(supabase, {
        ...
        refine: (q) => q.or(
          'condition.ilike.%refurb%,condition.ilike.%develop%,deal_type.ilike.%refurb%,deal_type.ilike.%develop%'
        ),
      });
      break;
    case 'urgent':
      result = await queryLiveLots(supabase, {
        ...
        order: { column: 'auction_date', ascending: true },
      });
      break;
    default:
  ...
  if (result.error) throw new Error(`findLotsByArchetype(${archetype}) failed: ${result.error.message}`);
  return result.data || [];
}
...
async function findLotsBySuperlative(superlative, opts = {}) {
  ...
  // Traditional live window only (same rationale as findLotsByArchetype).
  const result = await queryLiveLots(supabase, {
    select: LOT_COLS,
    limit,
    today,
    horizonEnd: horizon14,
    imageRequired: true,
    format: 'traditional',
    order: { column: def.order, ascending: def.ascending },
    refine: (q) => def.require(q),
  });
  if (result.error) throw new Error(`findLotsBySuperlative(${superlative}) failed: ${result.error.message}`);
  return result.data || [];
}
...
async function getFreshLotsForMaterial(limit = 6) {
  const today = todayUTC();
  const horizon = daysAheadUTC(21);
  const select = 'address, postcode, prop_type, beds, price, below_market, comparable_price, est_gross_yield, est_monthly_rent, condition, tenure, epc_rating, flood_risk, auction_date, house, score, deal_type, status, last_seen_at';

  // Live inventory = traditional upcoming (bounded) ∪ fresh MMOA/continuous.
  // Never treat 2099-12-31 as a content "auction this month".
  const result = await queryLiveLots(supabase, {
    select,
    limit: limit * 4,
    today,
  ...
  if (result.error) throw new Error(`getFreshLotsForMaterial failed: ${result.error.message}`);
...
async function getRecentBlogPostTitles(brand = 'auctionbrain', daysBack = 45) {
  ...
  // 'rejected' is included deliberately: a theme the editor just rejected as a
  // repeat must keep blocking similar resubmissions (the BoE rate-hold loop —
  // see docs/contentbrain-blog-repeats-incident.md). Brand-scoped because the
  // table briefly hosted misdirected rows from the other brand during that
  // incident; NULL-brand rows predate the brand column and are same-brand.
  const { data, error } = await client
    .from('blog_posts')
