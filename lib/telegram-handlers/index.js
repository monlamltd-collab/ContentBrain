...
const { isBlogReviewDay } = require('../review-cadence');
...
async function resendDraftReviewCards() {
  try {
    if (!isBlogReviewDay()) {
      console.log('[startup] blog review resend skipped — next review window is Tuesday/Thursday');
      return;
    }
    ...
    const { deliverBlogReview, wasBlogReviewDelivered } = require('../review-delivery');
    const { getSourceArticlesForPost } = require('../supabase');
    let sent = 0;
    for (const d of stale) {
      try {
        const brand = d.brand || 'auctionbrain';
        if (await wasBlogReviewDelivered(d.id, brand)) continue;
