...
async function selectAngles(brand, count, ctx) {
  ...
  const resp = await createClaudeLLM().messages.create({
    model: MODEL,
    max_tokens: 1200,
    system,
  ...
  if (!match) throw new Error(`No JSON in angle-selection response for ${brand}`);
  const angles = JSON.parse(match[0]).angles;
...
async function generateCopy(brand, templateType, briefText, contextBlock, author, angle) {
  ...
  const response = await createClaudeLLM().messages.create({
    model: MODEL,
    max_tokens: 700,
    system: systemPrompt,
