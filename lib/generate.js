...
async function generateCopy(brand, templateType) {
  const systemPrompt = getSystemPrompt(brand);
  const userPrompt = templatePrompts[templateType];
...
function assignPlatforms(templates) {
  // Distribute across platforms: first→facebook, second→linkedin, third→tiktok
  const platforms = ['facebook', 'linkedin', 'tiktok'];
  return templates.map((t, i) => ({ templateType: t, platform: platforms[i] }));
}
...
async function generateBatch() {
  ...
  for (const brand of Object.keys(brands)) {
    for (const { templateType, platform } of assignments) {
      ...
      const copy = await generateCopy(brand, templateType);
      posts.push({
        brand,
        platform,
