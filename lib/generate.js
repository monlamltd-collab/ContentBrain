...
// Hook + CTA pattern menus shared by reel and hook templates. Keeping the
// menu in one place (a) avoids prompt drift between the two templates and
// (b) lets the admin Intel tab chart pattern usage uniformly across both.
const HOOK_PATTERNS_MENU = `HOOK PATTERN MENU — pick exactly ONE that genuinely fits the available brand facts / seeds. Do NOT freestyle outside these patterns. Rotate: do not pick the same pattern as any of the recent headlines shown to you in context.
...
const CTA_PATTERNS_MENU = `CTA PATTERN MENU — pick ONE. Bare URL is forbidden. The CTA must promise something specific.
...
- Pattern names MUST be returned in hook_pattern (1-10) + cta_pattern (A-H) so the next generation can rotate.
