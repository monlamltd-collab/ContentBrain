...
router.post('/review', requireApiKey, async (req, res) => {
  ...
  try {
    ...
    // up by the Mon/Wed review-card sweep instead of interrupting the owner.
    if (shouldQueueReview(content_type)) {
      console.log(`[Review API] queued ${content_type} until Mon/Wed: "${title}"`);
      return res.status(202).json({
        ok: true,
        queued: true,
        message: 'Queued for the next Monday/Wednesday review window',
      });
    }
