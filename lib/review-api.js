...
const { deliverBlogReview } = require('./review-delivery');
const { getRecentBlogPostTitles } = require('./supabase');
const { isBlogReviewDay, shouldQueueReview } = require('./review-cadence');
...
router.post('/review', requireApiKey, async (req, res) => {
  ...
  if (content_type !== 'social' && req.query.force !== 'true') {
    try {
      // Exclude the submitted post's own row: engines save the draft to
      // blog_posts BEFORE posting here, so without this every submission
      // matches itself at 100% and auto-rejects (first fired 2026-06-12,
      // latent since the gate shipped).
      const recentPosts = (await getRecentBlogPostTitles(brand, 45))
        .filter(p => String(p.id) !== String(post_id));
  ...
  try {
    // Engines persist drafts before calling this endpoint. Off-cadence
    // submissions therefore remain safely queued in Supabase and are picked
    // up by the Tue/Thu review-card sweep instead of interrupting the owner.
    if (shouldQueueReview(content_type)) {
      console.log(`[Review API] queued ${content_type} until Tue/Thu: "${title}"`);
      return res.status(202).json({
        ok: true,
        queued: true,
        message: 'Queued for the next Tuesday/Thursday review window',
      });
    }
...
// Pure helpers exported for tests — the gate logic has now bitten three
// times (published-only memory, missing brand scope, self-match); keep it
// covered.
module.exports.titleWords = titleWords;
module.exports.jaccardSimilarity = jaccardSimilarity;
module.exports.findThemeMatch = findThemeMatch;
module.exports.isBlogReviewDay = isBlogReviewDay;
