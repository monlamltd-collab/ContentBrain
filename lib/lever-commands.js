...
async function handleLeverCommand(text /* , msg */) {
  ...
  // /boost [on | off | budget <£> | cap <£>] — paid follower-ad controls
  // Default state is OFF; this is the operator's phone off-switch for live
  // ad spend plus a quick spend/followers status read.
  if (head === '/boost') {
    const [, sub, rest] = cmdTokens(text, 2);
    const subLower = (sub || '').toLowerCase();
    const helpers = require('./social-engine/helpers');

    if (!sub || subLower === 'status') {
      const cfg = await runtimeConfig.getBoostConfig();
      let s = { committed_pence: 0, spent_pence: 0, follows: 0, runs: 0, active: 0 };
      try { s = await helpers.getBoostSummary(); } catch (_) { /* ledger read best-effort */ }
      const lines = [
        `<b>Paid follower ads</b> — ${cfg.enabled ? '🟢 ON' : '⚪️ OFF'}`,
        `Budget: £${(cfg.dailyBudgetPence / 100).toFixed(2)}/day per boost`,
        `Spend cap: £${(cfg.spendCapPence / 100).toFixed(2)} cumulative`,
        `Committed: £${(s.committed_pence / 100).toFixed(2)} · reconciled: £${(s.spent_pence / 100).toFixed(2)}`,
        `Followers gained: ${s.follows} · boosts: ${s.runs} (${s.active} live)`,
        ``,
        `<code>/boost on</code> · <code>/boost off</code> · <code>/boost budget 2</code> · <code>/boost cap 50</code>`,
      ];
      if (!process.env.MAKE_BOOST_WEBHOOK_URL) {
        lines.push(`\n⚠️ Make webhook not wired (MAKE_BOOST_WEBHOOK_URL unset) — ads can't fire yet even when ON.`);
      }
      await sendNotification(lines.join('\n'));
      return true;
    }
    if (subLower === 'on') {
      await runtimeConfig.setLever('global', 'boost.enabled', true);
      const warn = process.env.MAKE_BOOST_WEBHOOK_URL ? '' : ' Wire MAKE_BOOST_WEBHOOK_URL before ads can actually fire.';
      await sendNotification(`Paid follower ads <b>ON</b>.${warn}`);
      return true;
    }
    if (subLower === 'off') {
      await runtimeConfig.setLever('global', 'boost.enabled', false);
      await sendNotification(`Paid follower ads <b>OFF</b> — no new boosts will fire.`);
      return true;
    }
    if (subLower === 'budget') {
      const pounds = parseFloat(rest);
      if (!Number.isFinite(pounds) || pounds < 0.5 || pounds > 20) {
        throw new Error('Budget must be £0.50–£20/day, e.g. /boost budget 2');
      }
      await runtimeConfig.setLever('global', 'boost.daily_budget_pence', Math.round(pounds * 100));
      await sendNotification(`Per-boost daily budget set to <b>£${pounds.toFixed(2)}</b>/day.`);
      return true;
    }
    if (subLower === 'cap') {
      const pounds = parseFloat(rest);
      if (!Number.isFinite(pounds) || pounds < 1 || pounds > 1000) {
        throw new Error('Cap must be £1–£1000, e.g. /boost cap 50');
      }
      await runtimeConfig.setLever('global', 'boost.spend_cap_pence', Math.round(pounds * 100));
      await sendNotification(`Cumulative spend cap set to <b>£${pounds.toFixed(2)}</b>.`);
      return true;
    }
    throw new Error(`Unknown /boost subcommand "${sub}". Try: on, off, budget <£>, cap <£>, or just /boost for status.`);
  }
