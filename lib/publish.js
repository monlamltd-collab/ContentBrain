...
function getFbPage(brand) {
  // Resolve at call time rather than module load. This supports credential
  // rotation and makes isolated workers/tests safe when they initialise their
  // environment after importing another module that happens to load publish.
  const pages = {
    auctionbrain: {
  ...
  const page = pages[brand] || pages.auctionbrain;
...
async function publishAlbumToFacebook(page, imageFilenames, caption, brand) {
  ...
  // Guard: verify all files exist BEFORE any network calls to avoid orphan
  // unpublished media on the Page if a later file is missing.
...
//
// Outbound emails (track='outbound', channel='resend' — migration 008) take
// the Resend branch after Telegram approval. Per-track from-address resolved
// via lib/resend-from.js#getResendFrom (Phase F-2 — app_config > env >
// default). Suppression/filter pre-flight runs below as belt-and-braces.
...
async function publishToResend(post) {
  ...
  // Resolve per-track from-address (app_config > env > hardcoded default).
  // Falls back to the lender default if the track is unknown.
  let from;
  try {
    from = await getResendFrom(track);
  } catch (_e) {
    from = await getResendFrom('lender');
  }
