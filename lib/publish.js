...
async function publishToResend(post) {
  // TODO(coder):
  //   1. Require './resend' and './suppression' here (lazy to keep cold-start
  //      cost off the social-only path).
  //   2. Pull `to` from post.meta.contact_email (or look up the contact row by
  //      post.meta.contact_id and read its email).
  //   3. Pull `from` from a brand-keyed map RESEND_FROM[post.brand] with a
  //      sensible fallback — outbound from-addresses live on subdomains
  //      (e.g. simon@outreach.bridgematch.co.uk).
  //   4. Call sendOutbound({to, from, subject: post.copy_headline,
  //      body: post.copy_body, replyTo: from, headers: {...}}).
  //   5. Return { ok: true, channel: 'resend', resendId: <id> } — caller
  //      persists resendId into post.meta.resend_id.
  void post;
  throw new Error('publishToResend not implemented yet — see TODO(coder)');
}
