...
async function publishToFacebook(post) {
  ...
  // Phase G — Album post (multi-image carousel). Slots in BEFORE the single-
  // image branch so post.meta.album_images takes precedence over post.image_url
  // when both are set. If album_images is absent or < 2 entries, falls
  // through to the existing single-image branch — no regression for
  // existing flows.
  if (post.meta && Array.isArray(post.meta.album_images) && post.meta.album_images.length >= 2) {
    return publishAlbumToFacebook(page, post.meta.album_images, caption, post.brand);
  }
...
// Phase G — Publish a multi-image carousel as a Facebook Page post.
//
// Graph API protocol:
//   1. Upload each image to /<page-id>/photos with published=false to
//      get a media_fbid back (do NOT call /feed yet — these photos stay
//      unpublished until step 3 attaches them).
//   2. Build attached_media array from the returned media_fbids.
//   3. POST to /<page-id>/feed with message=<caption> and
//      attached_media=<json-array> — creates the single feed post that
//      owns all the previously-uploaded unpublished photos.
//
// Errors at any step throw; caller (publish cron) catches and routes to
// Telegram. Existing video / single-image / Resend / blog paths unchanged.
async function publishAlbumToFacebook(page, imageFilenames, caption, brand) {
  if (!Array.isArray(imageFilenames) || imageFilenames.length < 2) {
    throw new Error(`publishAlbumToFacebook: need >=2 images (got ${imageFilenames?.length || 0})`);
  }

  // Step 1 — upload each photo unpublished. fs.existsSync check BEFORE any
  // network call so a missing-file blows up early without leaving orphan
  // unpublished media on the Page.
  const mediaFbids = [];
  for (const filename of imageFilenames) {
    const imagePath = path.join(OUTPUT_DIR, filename);
    if (!fs.existsSync(imagePath)) {
      throw new Error(`publishAlbumToFacebook: missing file ${filename}`);
    }
    const form = new FormData();
    form.append('access_token', page.token);
    form.append('published', 'false');
    form.append('source', new Blob([fs.readFileSync(imagePath)]), filename);
    ...
    if (!res.ok) {
      // eslint-disable-next-line no-await-in-loop
      const err = await res.text();
      throw new Error(`Album photo upload (${filename}) failed (${res.status}): ${err.slice(0, 300)}`);
    }
    // eslint-disable-next-line no-await-in-loop
    const data = await res.json();
    if (!data.id) {
      throw new Error(`Album photo upload returned no id: ${JSON.stringify(data).slice(0, 200)}`);
    }
    mediaFbids.push(data.id);
  }

  // Step 2 — build attached_media JSON array.
  const attached_media = mediaFbids.map(id => ({ media_fbid: id }));

  // Step 3 — create the feed post that owns the album.
  const params = new URLSearchParams();
  params.append('access_token', page.token);
  params.append('message', caption);
  params.append('attached_media', JSON.stringify(attached_media));
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Album feed post failed (${res.status}): ${err.slice(0, 300)}`);
  }
  const data = await res.json();
  console.log(`  [Facebook] Album posted to ${brand} (${imageFilenames.length} images): ${data.id}`);
  return { ok: true, platform: 'facebook', postId: data.id };
}
...
// hard-coded.
//
// Phase D — 2026-05-25 — the only Resend domain verified today is
// `auctionbrain.co.uk` (apex); `outreach.bridgematch.co.uk` is still
// `not_started`. Per Simon's call, BOTH new Phase D tracks (broker +
// auction_house) default to sending from auctionbrain.co.uk to avoid the
// bridgematch DNS drama:
//   - broker: same Simon Deeming peer-to-peer framing; the dual-domain
//     explanation in the persona prompt handles the brand mismatch in the
//     first body line (mirrors the lender track's Phase B band-aid).
//   - auction_house: naturally brand-aligned (we ARE AuctionBrain).
// When `outreach.bridgematch.co.uk` is finally warmed, flip broker via the
// RESEND_FROM_BROKER env var on Railway — no code change needed.
//
// Lender default unchanged (Phase B set RESEND_FROM_LENDER in Railway env
// directly to handle the same DNS gap; not Phase D's surface to refactor).
const RESEND_FROM = {
  lender:        process.env.RESEND_FROM_LENDER       || 'Simon at BridgeMatch <outreach@outreach.bridgematch.co.uk>',
  broker:        process.env.RESEND_FROM_BROKER       || 'Simon Deeming <simon@auctionbrain.co.uk>',
  auction_house: process.env.RESEND_FROM_AUCTION_HOUSE || 'Simon Deeming <simon@auctionbrain.co.uk>',
};
...
async function publishToResend(post) {
  ...
  const { getRemainingBudget, isPaused } = require('./warming');
  const { buildHeaders: buildUnsubHeaders } = require('./unsubscribe');
  ...
  // Track-level pause kill-switch (deliverability incident etc.). Same
  // semantics as warming-budget-exhausted: leave the post 'approved',
  // cron picks it up when paused=false.
  if (await isPaused(track)) {
    console.warn(`[publishToResend] track '${track}' paused; deferring post ${post.id}`);
    return { ok: true, channel: 'resend', deferred: true, reason: 'track_paused' };
  }

  // Warming ramp budget check. Day-since-start defines today's cap; if we've
  // already hit it, defer (do not throw — the cron retries tomorrow).
  const budget = await getRemainingBudget(track);
  if (budget.remaining <= 0) {
    console.warn(`[publishToResend] warming cap reached for '${track}' (day ${budget.day}, cap ${budget.cap}, sent ${budget.sentToday}); deferring post ${post.id}`);
    return { ok: true, channel: 'resend', deferred: true, reason: 'warming_cap_reached', budget };
  }
  ...
  // Build the List-Unsubscribe header pair (RFC 2369 + RFC 8058) — gated on
  // PUBLIC_BASE_URL being set. If we don't have a public URL the one-click
  // endpoint can't be hit, so fall back to the mailto-only header from
  // resend.js. Better no broken link than a 404.
  const publicBase = process.env.PUBLIC_BASE_URL;
  const extraHeaders = publicBase ? buildUnsubHeaders(to, publicBase) : undefined;
  ...
  // Phase C — sequence-row side effect. Step 1 creates a fresh sequence row;
  // steps 2-4 advance the existing one (post.meta.sequence_id is set by the
  // cron handler when it queues the follow-up). Wrapped in try/catch: a
  // sequence-creation/advance failure must NOT roll back the publish — the
  // email has already gone out. Log loudly and continue; the Pipeline tab
  // surfaces sequences that fall out of sync for manual resolution.
  const sequenceStep = meta && meta.sequence_step;
  if (sequenceStep) {
    try {
      const seq = require('./sequence');
      if (sequenceStep === 1) {
        // Caller is expected to have populated meta.contact and meta.prospect
        // on the post (Phase B's queue-lender-batch does this). If absent,
        // skip — a sequence row needs both rows to satisfy the FK chain.
        const contact = meta.contact;
        const prospect = meta.prospect;
        if (contact && prospect) {
          await seq.createSequenceOnPublish(post, contact, prospect);
        } else {
          console.warn(`[publishToResend] post ${post.id} is step 1 but meta.contact/prospect missing; skipping sequence create`);
        }
      } else if (meta.sequence_id) {
        // Step N just SENT — bump the row's current_step / last_sent_at /
        // next_scheduled_at. NOT advanceSequence (that generates step N+1).
        await seq.bumpSequenceOnSendSuccess(meta.sequence_id, sequenceStep);
      } else {
        console.warn(`[publishToResend] post ${post.id} is step ${sequenceStep} but meta.sequence_id missing; cannot advance sequence`);
      }
    } catch (err) {
      console.warn(`[publishToResend] sequence side-effect failed for post ${post.id} (step ${sequenceStep}): ${err.message}`);
    }
  }

  console.log(`[publishToResend] published post ${post.id} -> ${to} (resend_id=${result.id})`);

  // Phase E — quiet Telegram receipt. Best-effort: a receipt-send failure
  // MUST NOT roll back the publish (the email has already gone out).
  try {
    const { sendOutboundReceipt } = require('./outbound-receipt');
    await sendOutboundReceipt({ subject, to });
  } catch (err) {
    console.warn(`[publishToResend] Telegram receipt for post ${post.id} failed: ${err.message}`);
  }
...
module.exports = { publish, publishToFacebook, publishReelToFacebook, publishAlbumToFacebook, publishToMake, publishBlogToFacebook, publishToResend };
