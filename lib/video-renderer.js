...
// Operator-tunable music choice (Studio settings → meta.music_file):
//   'none'            → silent video
//   '<track name>'    → that file from public/music/ (random fallback if gone)
//   unset             → random track (original behavior)
// post.musicFile (explicit caller prop) keeps absolute priority and its
// original pass-through semantics.
function resolveMusicFile(post) {
  if (post.musicFile !== undefined && post.musicFile !== null) return post.musicFile;
  const choice = post.meta && post.meta.music_file;
  if (!choice) return pickMusicFile();
  if (choice === 'none') return null;
  const base = path.basename(String(choice));
  if (fs.existsSync(path.join(MUSIC_DIR, base))) return `music/${base}`;
  console.warn(`[video-renderer] music_file "${choice}" not found — using random track`);
  return pickMusicFile();
}

// Duration is a first-class per-post setting. Resolution order:
//   1. post.overrideDurationSeconds — explicit caller override (lot=75,
//      Telegram revise) always wins
//   2. post.duration_seconds        — in-memory generation value
//   3. post.meta.duration_seconds   — persisted Studio/lever value
// Clamped to 3–90s (75s lot videos are the proven upper bound on Railway).
// Returns null when nothing is set → composition default applies.
const MIN_DURATION_SECONDS = 3;
const MAX_DURATION_SECONDS = 90;
function resolveDurationSeconds(post) {
  const raw = post.overrideDurationSeconds
    ?? post.duration_seconds
    ?? (post.meta && post.meta.duration_seconds);
  const n = Number(raw);
  if (!Number.isFinite(n) || n <= 0) return null;
  return Math.min(MAX_DURATION_SECONDS, Math.max(MIN_DURATION_SECONDS, n));
}
...
function buildProps(templateType, brandKey, post) {
  ...
  const base = {
    ...
    brandKey,
    musicFile: resolveMusicFile(post),
...
async function renderVideo(templateType, brandKey, post) {
  ...
  // First-class per-post duration (override > duration_seconds >
  // meta.duration_seconds, clamped 3–90s). Compositions read
  // durationInFrames reactively, so animations stretch/compress cleanly.
  const durationSeconds = resolveDurationSeconds(post);
  if (durationSeconds) {
    composition.durationInFrames = Math.round(durationSeconds * composition.fps);
  }
