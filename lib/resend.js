...
// ── Resend send/events wrapper (Phase B + Phase C) ────────────────────────
//
...
//          email.received   -> delegate to lib/inbound.handleInboundEmail
//                              (Phase C — native Resend inbound; webhook
//                              event was added on the existing webhook id
//                              6e1833b2-4f03-4a7e-96b4-831fd479983d on
//                              2026-05-25)
...
// Read env vars at call time, not module load. dotenv mutates process.env when
// the module is required, so capturing into a const at load makes tests that
// `delete process.env.RESEND_API_KEY` and reload the module ineffective (the
// const survives the env mutation). Reading at call time also lets ops rotate
// a key by updating Railway env without restarting the process.
let _client = null;
let _clientKey = null;  // remember which key spawned _client so a rotation rebuilds it
function getClient() {
...
//
// Resend webhooks are signed by Svix. Verification rules:
//   1. Secret has a `whsec_` prefix; the bytes after that are base64-encoded
//      and must be base64-decoded before HMAC.
//   2. The signed payload is NOT the body alone — it is
//        `${svix-id}.${svix-timestamp}.${rawBody}`
//      concatenated as a UTF-8 string (with the body as raw bytes).
//   3. The signature is base64-encoded, NOT hex.
//   4. The `svix-signature` header is space-separated `v1,<sig>` parts (key
//      rotation can yield multiple). Accept if ANY matches.
//   5. Reject when `svix-timestamp` is more than 5 minutes off real time
//      (replay protection).
//
// Reference: https://docs.svix.com/receiving/verifying-payloads/how-manual
//
// Fails closed (throws) when RESEND_WEBHOOK_SECRET is unset so an
// unconfigured deployment can't accept forged events.
...
function verifySignature(rawBody, headers) {
  ...
  const svixId = headers && (headers['svix-id'] || headers['Svix-Id']);
  const svixTimestamp = headers && (headers['svix-timestamp'] || headers['Svix-Timestamp']);
  const sigHeader = headers && (headers['svix-signature'] || headers['Svix-Signature'] || headers['resend-signature']);

  if (!svixId || !svixTimestamp || !sigHeader) {
    throw new Error('Webhook verification: missing svix-id / svix-timestamp / svix-signature header');
  }

  // Replay protection — clock skew tolerance ±5 min.
  const ts = parseInt(svixTimestamp, 10);
  if (!Number.isFinite(ts)) {
    throw new Error('Webhook verification: svix-timestamp is not numeric');
  }
  const now = Math.floor(Date.now() / 1000);
  if (Math.abs(now - ts) > SVIX_TIMESTAMP_TOLERANCE_SECONDS) {
    throw new Error(`Webhook verification: svix-timestamp outside ±${SVIX_TIMESTAMP_TOLERANCE_SECONDS}s tolerance (drift ${now - ts}s)`);
  }
...
async function handleWebhook(rawBody, headers) {
  ...
  switch (type) {
    ...
    case 'email.received': {
      // Phase C — native Resend inbound. The webhook payload is metadata
      // only; lib/inbound fetches the full body via the SDK and runs
      // classify + dispatch (sequence transition, suppression, Telegram).
      // Lazy-require to keep the social hot path free of Anthropic /
      // sequence module loads.
      try {
        const { handleInboundEmail } = require('./inbound');
        const result = await handleInboundEmail(data);
        actions.push(`inbound handled: reply=${result && result.replyId} intent=${result && result.intent}`);
      } catch (err) {
        actions.push(`inbound handler failed: ${err.message}`);
        console.error(`[resend webhook] email.received handler error: ${err.message}`);
        // A failed inbound means a real prospect reply went unprocessed —
        // alert Telegram so it isn't lost in Railway logs. Own try/catch so
        // a Telegram outage can't turn a logged error into a webhook 500.
        try {
          const { sendNotification } = require('./telegram');
          await sendNotification(
            `<b>Inbound email handler FAILED</b>\n` +
            `From: ${data?.from || 'unknown'}\n` +
            `Subject: ${data?.subject || '(none)'}\n` +
            `email_id: ${data?.email_id || data?.id || 'n/a'}\n` +
            `Error: ${String(err.message || err).slice(0, 200)}\n\n` +
            `The reply was NOT classified or dispatched — handle manually.`
          );
        } catch (tgErr) {
          console.error(`[resend webhook] failure alert also failed: ${tgErr.message}`);
        }
      }
      break;
    }
