...
function defaultChromiumLibDir(env = process.env) {
  const installRoot = env.CHROMIUM_LIB_ROOT || path.join(env.HOME || '/data', '.local', 'chromium-libs');
  return path.join(installRoot, 'root', 'usr', 'lib', 'x86_64-linux-gnu');
}

function findHeadlessShell() {
  if (process.env.PUPPETEER_EXECUTABLE_PATH) return process.env.PUPPETEER_EXECUTABLE_PATH;
  const root = path.join(process.env.HOME || '/data', '.cache', 'puppeteer', 'chrome-headless-shell');
  if (!fs.existsSync(root)) return undefined;
  const versions = fs.readdirSync(root).filter(name => name.startsWith('linux-')).sort().reverse();
...
function getBrowserLaunchOptions({ executablePath = findHeadlessShell(), env: sourceEnv = process.env } = {}) {
  const libDir = sourceEnv.CHROMIUM_LIB_DIR || defaultChromiumLibDir(sourceEnv);
  const env = { ...sourceEnv };
  if (fs.existsSync(libDir)) {
    env.LD_LIBRARY_PATH = [libDir, sourceEnv.LD_LIBRARY_PATH].filter(Boolean).join(':');
  }
  return {
    headless: true,
    executablePath,
    ...
    args: [
      ...
      // Docker's /dev/shm is commonly limited to 64 MB. Use /tmp instead so
      // concurrent cron renders do not crash Chromium during launch.
...
module.exports = { renderPost, renderAlbum, renderTestSuite, buildHtml, getBrowserLaunchOptions, findHeadlessShell, defaultChromiumLibDir };
