 ...
 * @param {string} url
 * @param {object} [opts]
 * @param {Array}  [opts.formats]   e.g. ['markdown'], ['rawHtml'] or [{ type:'json', schema, prompt }]
 * @param {string} [opts.proxy]     'basic' (1cr) | 'enhanced' (5cr, residential/anti-bot) | 'auto'.
 *                                  Omit for the account default. Used sparingly to bypass anti-bot walls.
 * @param {number} [opts.timeoutMs] request timeout (default 60s)
 * @returns {Promise<object>} the Firecrawl `data` object ({ markdown, rawHtml, json, metadata, ... })
 */
async function firecrawlScrape(url, { formats = ['markdown'], proxy, timeoutMs = 60000 } = {}) {
  ...
  try {
    const res = await fetch(`${API_BASE}/scrape`, {
      ...
      body: JSON.stringify({ url, formats, ...(proxy ? { proxy } : {}) }),
