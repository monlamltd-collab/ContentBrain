...
async function enrichDomain(domain) {
  ...
  const url = `${HUNTER_BASE_URL}/domain-search?domain=${encodeURIComponent(clean)}&api_key=${encodeURIComponent(HUNTER_API_KEY)}`;
...
async function enrichEmail(email) {
  ...
  const url = `${HUNTER_BASE_URL}/email-verifier?email=${encodeURIComponent(clean)}&api_key=${encodeURIComponent(HUNTER_API_KEY)}`;
  const res = await fetch(url);
  const bodyText = await res.text();
