...
function renderAiMediaSection(snapshot) {
  const hf = snapshot.higgsfield || { perBrand: {}, daily_cap: 20, auto_mode: false };
  const brandBlocks = snapshot.brands.map(brand => {
    const v = hf.perBrand[brand] || { style_prefix: '', default_aspect: '' };
    const aspects = ['', '1:1', '9:16', '16:9', '4:5'];
    const aspectOpts = aspects.map(a =>
      `<option value="${escAttr(a)}"${v.default_aspect === a ? ' selected' : ''}>${a || 'Auto (reel 9:16, else 1:1)'}</option>`).join('');
    return `<div class="design-brand-card">
  <div class="design-brand-head"><strong>${escHtml(snapshot.perBrand[brand].name || brand)}</strong></div>
  <form class="design-field" hx-post="/dashboard/design/higgsfield/${escAttr(brand)}" hx-target="closest .design-field" hx-swap="outerHTML">
    <label>House style prefix <span class="hint">Prepended to every AI-drafted prompt for this brand.</span></label>
    <textarea name="style_prefix" rows="2" maxlength="400">${escHtml(v.style_prefix)}</textarea>
    <label>Default aspect</label>
    <select name="default_aspect">${aspectOpts}</select>
    <div class="field-row">
      <button type="submit" class="btn btn-save">Save</button>
      ${savedFlash()}
    </div>
  ...
  return sectionShell('aimedia', 'AI media (Higgsfield)',
    'Image and animated-reel generation in the Studio tab. Manual only — every generation is a button you press.',
    `<div class="design-brand-grid">${brandBlocks}</div>
<form class="mix-form" hx-post="/dashboard/design/higgsfield-cap" hx-target="#design-aimedia" hx-swap="outerHTML">
  <div class="field-row">
    <label class="mix-item"><span>Daily generation cap</span>
      <input type="number" name="daily_cap" min="1" max="200" value="${Number(hf.daily_cap) || 20}">
    </label>
    <label class="toggle" title="Coming soon — generates AI media automatically in the daily run.">
      <input type="checkbox" disabled ${hf.auto_mode ? 'checked' : ''}>
      <span class="hint">Automatic generation in daily run — coming soon</span>
    </label>
    <button type="submit" class="btn btn-save">Save cap</button>
    ${savedFlash()}
  </div>
</form>`);
}
