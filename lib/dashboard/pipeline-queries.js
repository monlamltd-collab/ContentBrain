...
//
// House style: British English in any user-facing strings (the queries
// themselves don't return UI text — that lives in the renderers).
 ...
 *
 * Two reads merged in JS by COALESCE(reply.created_at, seq.last_sent_at,
 * seq.created_at) DESC, capped at NEEDS_ATTENTION_CAP.
 *
 * @param {object} [opts]
 * @param {number|'all'} [opts.windowDays=7]
 * @param {string[]} [opts.intents]
 * @returns {Promise<Array<{kind:'reply'|'paused-sequence', ts:string, reply?:object,
 *   sequence?:object, contact:object, prospect:object, jump_url:string}>>}
 */
async function getNeedsAttention(opts) {
  ...
  // 1. Replies — joined with contact + prospect + sequence.
  // Filter: classified_intent IN (intents) AND created_at >= window.from.
  // Note: the brief says "requires_human=true" OR "intent in (intents)" —
  // architect chose intent filter as the primary signal (matches the
  // design doc Section A semantics; requires_human is set by lookupAction
  // for exactly these intents anyway).
  ...
  if (pausedErr) throw new Error(`getNeedsAttention paused-sequences failed: ${pausedErr.message}`);
...
/**
 * Active sequences, paginated. ORDER BY next_scheduled_at ASC. `track='all'`
 * disables the filter. Returns `{rows, page, pageSize, hasMore}` — hasMore
 * is heuristic (rows.length === pageSize).
 *
 * @param {object} [opts]
...
/**
 * Interleaved outbound sends + replies, newest first. Two reads merged in JS.
 *
 * @param {object} [opts]
 * @param {number|'all'} [opts.windowHours=24]
 * @param {number} [opts.limit=40]
 * @returns {Promise<Array<{kind:'send'|'reply', at:string, label:string,
 *   contact_email:string, intent?:string, subject?:string}>>}
 */
...
/**
 * Fetch one reply row with joined contact + prospect + sequence. Used by
 * the action handlers in routes/dashboard/pipeline.js.
 *
