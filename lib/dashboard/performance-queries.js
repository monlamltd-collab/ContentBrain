...
async function getMetrics({ windowDays = 7 } = {}) {
  ...
  // PR4 — audience growth series for the sparkline. Degrades to [] when
  // the table is missing or the query fails (same tolerance pattern as
  // _getFollowerDelta). Social engine is AB-only, so one brand.
  let audience_series = [];
  try {
    const { getAudienceSeries } = require('../social-engine/helpers');
    audience_series = await getAudienceSeries({ brand: SOCIAL_BRAND, days: 30 });
  } catch (err) {
    console.warn(`[performance] getAudienceSeries degraded to []: ${err.message}`);
  }
...
function renderPerformanceFragment({ windowDays, metrics }) {
  ...
  const audienceHtml = renderAudienceSection(metrics.audience_series);

  return `${weeklyReviewHtml}${audienceHtml}<section class="perf-section">
...
/**
 * Render the audience-growth sparkline section (PR4 surface). Inline SVG —
 * pure string-built polyline, no chart dependency. Returns '' when the
 * series is empty or has fewer than 2 points (nothing to draw).
 */
function renderAudienceSection(series) {
  const points = (Array.isArray(series) ? series : [])
    .map(r => ({ date: r.recorded_at, n: Number(r.followers_count) }))
  ...
  const coords = points.map((p, i) => {
    ...
    const y = PAD + (1 - (p.n - min) / span) * (H - 2 * PAD);
  ...
  const deltaClass = delta >= 0 ? 'perf-delta-up' : 'perf-delta-down';

  return `<section class="perf-section perf-audience">
  <h3>Audience growth <span class="perf-window">— followers, last 30 days</span></h3>
  <div class="perf-audience-row">
    <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Follower count sparkline">
      <polyline fill="none" stroke="currentColor" stroke-width="1.5" points="${coords}" />
    </svg>
    <dl class="perf-content-stats">
      <div><dt>Start</dt><dd>${fmtNum(first.n)}</dd></div>
      <div><dt>Now</dt><dd>${fmtNum(last.n)}</dd></div>
      <div><dt>Change</dt><dd class="${deltaClass}">${deltaTxt}</dd></div>
    </dl>
  </div>
</section>`;
}
