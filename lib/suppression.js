...
async function isSuppressed(email) {
  ...
  // Dashboard lever gate — the ONE central read-side check (gates both the
  // publishToResend pre-flight and sendOutbound's internal belt-and-braces
  // call). Writes (bounce/complaint adds) stay always-on. Lazy-require to
  // avoid a module cycle. Lever read failure ⇒ enabled (fail safe).
  try {
    const { isSuppressionCheckEnabled } = require('./runtime-config');
    if (!(await isSuppressionCheckEnabled())) {
      console.warn(`[suppression] CHECK DISABLED via dashboard lever — allowing ${lower}`);
      return { suppressed: false, match: null, reason: null, level: null, disabled: true };
    }
  } catch (leverErr) {
    // Treat as enabled — never let a config read outage skip suppression.
  }
