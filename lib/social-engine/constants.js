...
const BOOST_OBJECTIVE = 'OUTCOME_ENGAGEMENT'; // ODAX top-level objective for follower growth
// Follower growth: Meta optimises an OUTCOME_ENGAGEMENT ad for page follows
// when optimization_goal=PAGE_LIKES and promoted_object names the Page. The
// retired PAGE_LIKES *campaign objective* is NOT this — it's the optimization
// goal under engagement. Make maps these two fields onto the adset.
const BOOST_FOLLOWER_OPTIMIZATION_GOAL = 'PAGE_LIKES';
// Hard cumulative ceiling across all non-failed boost runs — the safety rail
// for live spend. £50 default for the initial follower test; tunable via the
// boost.spend_cap_pence lever (/boost cap). requestBoost refuses to fire once
// committed exposure would cross this.
const DEFAULT_BOOST_SPEND_CAP_PENCE = 5000; // £50
...
// PR4 — interest narrowing for non-regional niche tags. When a boosted
// post carries one of these tags, deriveAudienceSpec swaps the default
// interest list for the tag's dedicated set (geo + age stay default).
const NICHE_INTEREST_OVERRIDES = Object.freeze({
  'yield-8plus':         Object.freeze(['Buy-to-let', 'Property investment']),
  'yield-10plus':        Object.freeze(['Buy-to-let', 'Property investment']),
  'refurb-projects':     Object.freeze(['Property development', 'Home improvement']),
  'below-market-20plus': Object.freeze(['Property investment', 'Real estate auction']),
});
