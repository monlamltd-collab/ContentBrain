 ...
 * Traditional live window only (real dates) — roundups are date/urgency framed.
...
async function findLotsByRegion(prefixes, opts = {}) {
  ...
  const horizon = daysAheadUTC(daysAhead);
  ...
  const result = await queryLiveLots(supabase, {
    ...
    refine: (q) => q.or(orClause),
  });
  if (result.error) throw new Error(`findLotsByRegion(${list.join('|')}) failed: ${result.error.message}`);
  return result.data || [];
}
...
async function findLotsByYieldBand(opts = {}) {
  ...
  const result = await queryLiveLots(supabase, {
    ...
    refine: (q) => {
      q = q.gte('est_gross_yield', minYield);
      if (Number.isFinite(maxYield)) q = q.lte('est_gross_yield', maxYield);
      return q;
    },
  });
  if (result.error) throw new Error(`findLotsByYieldBand failed: ${result.error.message}`);
  return result.data || [];
}
...
async function findLotsByDealTag(dealTag, opts = {}) {
  ...
  /** @type {(q:any)=>any} */
  let refine;
  /** @type {{column:string, ascending:boolean}} */
  let order = { column: 'score', ascending: false };

  switch (dealTag) {
    case 'refurb-projects':
      refine = (q) => q.or('condition.ilike.%refurb%,condition.ilike.%develop%,deal_type.ilike.%refurb%,deal_type.ilike.%develop%');
      break;
    case 'vacant-possession':
      refine = (q) => q.eq('vacant', true);
      break;
    case 'below-market-20plus':
      refine = (q) => q.gte('below_market', 20);
      order = { column: 'below_market', ascending: false };
      break;
    case 'prop-terraced':
      refine = (q) => q.ilike('prop_type', '%terraced%');
      break;
    case 'prop-commercial':
      refine = (q) => q.ilike('prop_type', '%commercial%');
      break;
    case 'prop-flat':
      refine = (q) => q.ilike('prop_type', '%flat%');
      break;
    default:
  ...
  if (result.error) throw new Error(`findLotsByDealTag(${dealTag}) failed: ${result.error.message}`);
  return result.data || [];
}

/**
 * Aggregate stats over live lots. Used by curiosity-gap and data-shock.
 * The aggregation happens client-side (one capped fetch).
 * @param {object} opts
 *   - groupBy: null|'postcode_area'|'prop_type'|'house'
 *   - upcomingOnly: bool (default true) — when true, traditional live bounds only
 *                   (real auction_date >= today and < 2090; never sentinel 2099)
...
async function aggregateLotStats(opts = {}) {
  ...
  if (upcomingOnly) {
    // Safe traditional window — do NOT use bare gte(today) (includes 2099 MMOA).
    query = applyTraditionalLiveWindow(query, { today: todayUTC() });
  }
  if (minScore != null) {
...
// ── Spend ledger (real-money safety) ──────────────────────────────────────

/**
 * Total COMMITTED ad spend in pence across every non-failed boost run.
 *
 * "Committed" (not just reconciled) is deliberate: a boost reserves its full
 * budget the moment it fires, but Meta's spend only flows back via the daily
 * reconcile hours later. Counting committed exposure — the greater of actual
 * reconciled spend and the budgeted maximum (daily_budget × days) — means many
 * £2 boosts can't slip past the cap in the window before Meta reports back.
 * Failed runs are excluded (they never spent).
 * @returns {Promise<number>}  committed pence
 */
async function getCommittedBoostSpendPence() {
  const { data, error } = await supabase
    .from('boost_runs')
    .select('status, spend_pence, daily_budget_pence, duration_hours')
    .in('status', ['pending', 'active', 'complete']);
  if (error) throw new Error(`getCommittedBoostSpendPence failed: ${error.message}`);
  let total = 0;
  for (const r of data || []) {
    const days = Math.max(1, Math.ceil((r.duration_hours || 24) / 24));
    const budgeted = (r.daily_budget_pence || 0) * days;
    const spent = Number.isFinite(r.spend_pence) ? r.spend_pence : 0;
    total += Math.max(budgeted, spent);
  }
  return total;
}

/**
 * Aggregate boost ledger for the /boost status view: committed spend,
 * reconciled spend, follows gained, and run counts by status.
 * @returns {Promise<{committed_pence:number, spent_pence:number, follows:number, runs:number, active:number}>}
 */
async function getBoostSummary() {
  const { data, error } = await supabase
    .from('boost_runs')
    .select('status, spend_pence, daily_budget_pence, duration_hours, ad_new_follows');
  if (error) throw new Error(`getBoostSummary failed: ${error.message}`);
  const rows = data || [];
  let committed = 0, spent = 0, follows = 0, active = 0;
  for (const r of rows) {
    if (r.status === 'failed') continue;
    const days = Math.max(1, Math.ceil((r.duration_hours || 24) / 24));
    committed += Math.max((r.daily_budget_pence || 0) * days, Number.isFinite(r.spend_pence) ? r.spend_pence : 0);
    spent += Number.isFinite(r.spend_pence) ? r.spend_pence : 0;
    follows += Number.isFinite(r.ad_new_follows) ? r.ad_new_follows : 0;
    if (r.status === 'pending' || r.status === 'active') active += 1;
  }
  return { committed_pence: committed, spent_pence: spent, follows, runs: rows.length, active };
}
