...
// ── Breakout signals (PR2 stubs; PR4 will populate) ────────────────────────

/**
 * Returns true if a posts row from the last BREAKOUT_OVERRIDE_HOURS hours
 * has meta.breakout_score >= BREAKOUT_THRESHOLD and its niche_tag has not
 * been re-featured since. PR2: returns false unconditionally (breakout_score
 * writes ship in PR4).
 * @returns {Promise<boolean>}
 */
async function isBreakoutActive() {
  // PR2 stub. Interface present so PR4 can flip on without changing the
  // orchestrator. Reads BREAKOUT_OVERRIDE_HOURS + BREAKOUT_THRESHOLD only to
  // satisfy linters that flag unused imports — they will be the live query's
  // input in PR4.
  void BREAKOUT_OVERRIDE_HOURS;
  void BREAKOUT_THRESHOLD;
  return false;
}

/**
 * Returns breakout amplification tag descriptors for decideType().
 * PR2 stub: empty array. PR4 will populate from posts.meta.breakout_score.
