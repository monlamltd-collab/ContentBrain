...
//
// ── Replay protection (PR4, opt-in via MAKE_HMAC_TS=1) ──
//
// When MAKE_HMAC_TS=1, the digest is computed over `${timestamp}.${body}`
// (Stripe-style) and the timestamp travels as X-CB-Timestamp (outbound) /
// X-Make-Timestamp (inbound). Verification REQUIRES the timestamp header
// and rejects when |now - ts| > 300s — never header-optional, since an
// attacker would simply omit it.
//
// ROLLOUT (two-system change — see .env.example MAKE_HMAC_TS):
//   1. Deploy this code with the flag UNSET (exact legacy behaviour).
//   2. Update BOTH Make scenarios (ub-social-boost verify + callback sign,
//      ub-social-boost-reconcile sign) to the `${ts}.${body}` scheme.
//   3. Set MAKE_HMAC_TS=1 in Railway.
...
function tsEnabled() {
  return process.env.MAKE_HMAC_TS === '1';
}

function toBuf(body) {
  return Buffer.isBuffer(body) ? body : Buffer.from(body == null ? '' : body);
}

function computeHexDigest(secret, body) {
  return crypto.createHmac('sha256', secret).update(toBuf(body)).digest('hex');
}

function computeHexDigestTs(secret, timestamp, body) {
  const signed = Buffer.concat([Buffer.from(`${timestamp}.`), toBuf(body)]);
 ...
 * @returns {string|{signature: string, timestamp: number}}
 *   Flag off (legacy): the header string "sha256=abc123...".
 *   MAKE_HMAC_TS=1: { signature, timestamp } — caller sets BOTH
 *   X-CB-Signature and X-CB-Timestamp headers.
...
function signOutbound(body) {
  ...
  return { signature: `sha256=${computeHexDigestTs(secret, timestamp, body)}`, timestamp };
}
...
function verifyDigestHeader(secret, rawBody, sigHeader) {
  ...
  return timingSafeHeaderCompare(expectedHeader, sigHeader);
}

/**
 * Timestamped variant — digest over `${ts}.${body}`, REQUIRES the timestamp
 * and rejects outside the ±REPLAY_WINDOW_SECS window.
 */
function verifyDigestHeaderTs(secret, rawBody, sigHeader, tsValue) {
  if (!sigHeader) {
    throw new Error('Webhook verification: missing signature header');
  }
  if (!tsValue) {
    throw new Error('Webhook verification: missing timestamp (MAKE_HMAC_TS is enabled)');
  }
  const ts = parseInt(tsValue, 10);
  if (!Number.isFinite(ts)) {
    throw new Error('Webhook verification: invalid timestamp');
  }
  const skew = Math.abs(Math.floor(Date.now() / 1000) - ts);
  if (skew > REPLAY_WINDOW_SECS) {
    throw new Error(`Webhook verification: timestamp outside ±${REPLAY_WINDOW_SECS}s replay window`);
  }
  const expectedHeader = `sha256=${computeHexDigestTs(secret, ts, rawBody)}`;
  return timingSafeHeaderCompare(expectedHeader, sigHeader);
}

function timingSafeHeaderCompare(expectedHeader, sigHeader) {
...
function verifyInbound(rawBody, headers) {
  ...
  if (tsEnabled()) {
    const tsHeader = headers && (headers['x-make-timestamp'] || headers['X-Make-Timestamp']);
    return verifyDigestHeaderTs(secret, rawBody, sigHeader, tsHeader);
  }
 ...
 */
function verifyOutbound(rawBody, headers, querySig, queryTs) {
  const secret = process.env.MAKE_WEBHOOK_SECRET;
  if (!secret) {
  ...
  if (tsEnabled()) {
    const headerTs = headers && (headers['x-cb-timestamp'] || headers['X-CB-Timestamp']);
    return verifyDigestHeaderTs(secret, rawBody, sig, headerTs || queryTs);
  }
