...
async function requestBoost(post, fb_post_id) {
  ...
  // live on FB; the boost is amplification, not critical path. We POST
  // the signed request, the row sits at 'pending', and Make's callback
  // to /api/social-boost-callback flips it to 'active' (or 'failed') async.
  // Stub for coder — build the payload object, JSON.stringify, sign via
  // signOutbound, fetch POST with x-cb-signature header. On non-2xx,
  // markBoostFailed(row.id, `Make webhook ${status}: ${truncated body}`)
  // and return { boost_run_id: row.id, fired_webhook: false }. On 2xx,
  // return { boost_run_id: row.id, fired_webhook: true }. NEVER throw —
  // server.js's caller try/catch already handles errors via Telegram.
  // See payload contract in phase-g3-design.md §3.5 (request_id, post_id,
  // fb_post_id, page_id, objective, daily_budget_pence, duration_hours,
  // audience_spec, callback_url).
  throw new Error('NOT_IMPLEMENTED: webhook fire body — coder to wire signOutbound + fetch');
}
