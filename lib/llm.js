...
async function llmJson(llm, createArgs, { label = 'llm', retries = 1, validate = null } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    const args = attempt === 0 ? createArgs : {
      ...createArgs,
      messages: [
        ...createArgs.messages.slice(0, -1),
        {
          ...createArgs.messages[createArgs.messages.length - 1],
          content: createArgs.messages[createArgs.messages.length - 1].content
            + '\n\nIMPORTANT: Return ONLY the JSON object, complete with every required field — no prose, no markdown fences.'
            + (lastErr ? ` The previous response was rejected because: ${lastErr.message.slice(0, 240)}` : ''),
        },
      ],
    };
    const resp = await llm.messages.create(args);
    const text = (resp.content || []).find(b => b.type === 'text')?.text || '';
    try {
      const parsed = parseLLMJson(text, { label });
