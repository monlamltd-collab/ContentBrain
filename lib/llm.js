...
// Cheap model routed via OpenRouter. Override with OPENROUTER_MODEL env var.
// On failure (e.g. a retired model 404ing, as google/gemini-2.0-flash-exp:free
// did) the call cascades through OPENROUTER_FALLBACK_MODELS — kept provider-
// diverse so a single-provider outage doesn't take the cheap path down.
...
// ── OpenRouter ────────────────────────────────────────────────────────────
async function _callOpenRouterModel(model, { system, messages, maxOutputTokens }) {
...
// Cascade across [primary, ...fallbacks]. On any per-model failure (retired
// model 404, empty choices, transient blip) advance to the next model; throw
// the last error only when every model has failed. The surrounding retry
// wrapper in createLLM() then re-attempts the whole chain for transient cases.
async function _callOpenRouter({ system, messages, maxOutputTokens }) {
  const models = [MODEL, ...FALLBACK_MODELS];
  let lastErr;
  for (const model of models) {
    try {
      return await _callOpenRouterModel(model, { system, messages, maxOutputTokens });
    } catch (err) {
      lastErr = err;
      if (models.length > 1) {
        console.warn(`[llm] model ${model} failed (${err.message.slice(0, 140)}) — trying next`);
      }
    }
  }
  throw lastErr;
 ...
 * Throws with labeled context (raw text slice) on no-match or parse failure
 * so failures are diagnosable from logs instead of bare SyntaxErrors.
 */
function parseLLMJson(text, { label = 'llm' } = {}) {
  const raw = typeof text === 'string' ? text : '';
  const match = raw.match(/\{[\s\S]*\}/);
  if (!match) {
    throw new Error(`[${label}] no JSON object in LLM response. Raw: ${raw.slice(0, 300)}`);
  }
  try {
    return JSON.parse(match[0]);
  } catch (err) {
    throw new Error(`[${label}] JSON parse failed: ${err.message}. Raw: ${match[0].slice(0, 300)}`);
  }
}

/**
 * Call an LLM and parse JSON from its response, retrying once on parse
 * failure with an explicit "return only JSON" reminder appended.
 */
async function llmJson(llm, createArgs, { label = 'llm', retries = 1, validate = null } = {}) {
  let lastErr;
  let lastRaw = '';
  const baseMessages = [...createArgs.messages];
  for (let attempt = 0; attempt <= retries; attempt++) {
    const messages = attempt === 0 ? baseMessages : [
      ...
      {
        role: 'user',
        content: 'Your previous answer was not valid JSON. Convert your previous answer into the required JSON schema from my original request. Preserve the facts and wording where possible. Return ONLY the JSON object, complete with every required field — no markdown fences, commentary, or preamble.'
          + (lastErr ? ` The previous response was rejected because: ${lastErr.message.slice(0, 240)}` : ''),
      },
    ];
    const resp = await llm.messages.create({ ...createArgs, messages });
    lastRaw = (resp.content || []).find(b => b.type === 'text')?.text || '';
    try {
      const parsed = parseLLMJson(lastRaw, { label });
      if (validate) validate(parsed);
      return parsed;
    } catch (err) {
      lastErr = err;
      if (attempt < retries) {
        console.warn(`[${label}] JSON attempt ${attempt + 1}/${retries + 1} failed (${err.message.slice(0, 160)}) — retrying`);
      }
    }
  }
  throw lastErr;
...
module.exports = { createLLM, createClaudeLLM, MODEL, CLAUDE_MODEL, parseLLMJson, llmJson };
