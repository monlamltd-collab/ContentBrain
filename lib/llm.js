 ...
 */
async function llmJson(llm, createArgs, { label = 'llm', retries = 1 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    const text = (resp.content || []).find(b => b.type === 'text')?.text || '';
    try {
      return parseLLMJson(text, { label });
    } catch (err) {
      lastErr = err;
      if (attempt < retries) {
        console.warn(`[${label}] JSON parse attempt ${attempt + 1}/${retries + 1} failed — retrying with JSON-only reminder`);
      }
    }
  }
  throw lastErr;
