...
// Lot copy is the freshest, highest-value content — generate it on Claude
// Sonnet (with extended thinking) for quality. createClaudeLLM ignores `model`.
...
function buildLotSummary(lot) {
  const lines = [
    ...
    lot.comparable_price ? `Street average comp: ${gbp(lot.comparable_price)}` : null,
...
async function generateLotContent({ lot, archetype, llm = createClaudeLLM() }) {
...

{
  ...
  const parsed = await llmJson(llm, {
    ...
    retries: 2,
    validate: value => {
      for (const key of ['hook_headline', 'voiceover_script', 'caption_facebook']) {
        if (typeof value[key] !== 'string' || !value[key].trim()) {
          throw new Error(`missing or empty required field: ${key}`);
        }
      }
      if (!Array.isArray(value.key_bullets) || value.key_bullets.length === 0
          || !value.key_bullets.every(item => typeof item === 'string' && item.trim())) {
        throw new Error('missing or empty required field: key_bullets');
      }
    },
  });
