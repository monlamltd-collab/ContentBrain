#!/usr/bin/env node
//
// scripts/queue-auction-house-batch.js
//
// Generate N cold-open outbound emails for auction-house contacts and
// queue them in Telegram for review. N defaults to the auction_house
// track warming day-1 cap (10).
//
// Selection: any contact at prospects where type='auction_house',
// ordered by email. Excludes already-queued contacts so re-running is safe.
//
// Usage:
//   node scripts/queue-auction-house-batch.js          (uses today's warming cap)
//   node scripts/queue-auction-house-batch.js 5        (queue exactly 5)
...
const TRACK = 'auction_house';
const TRACK_LABEL = 'AUCTION HOUSE OUTBOUND';
...
async function pickBatch(count) {
  ...
  // it the alphabetical email ordering brings 60+ lender contacts in first
  // and consumes the SQL limit before we ever see an auction_house row.
  const { data, error } = await supabase
    .from('contacts')
    .select(`
